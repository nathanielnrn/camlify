(*(* TODO: Implement according to music_data.mli *) open
  ...
  to_string; artist = (if song |> member "artist" = `Null then None else
  song |> member "artist" |> to_string_option); album = (if song |>
  ...
  let to_x = to_interface x let pushed = Yojson.pretty_to_string to_x*)
